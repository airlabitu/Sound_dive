...
// Format: Name, Color, X, Y, Z, Distance Threshold and Channel (track)
  ...
  Sound cableDroneScan = new Sound("Cable Drone Scan", color(255, 255, 255), cableDrone.x, cableDrone.y, cableDrone.z, w_width/2, 9);
     //cables
  Sound dataCable1 = new Sound("Data Cable1", color(255,0,0), w_width-30, 0, -10, 60, 5);
  Sound dataCable2 = new Sound("Data Cable2", color(255,0,0), w_width-60, 0, -10, 60, 5);
  Sound dataCable3 = new Sound("Data Cable3", color(255,0,0), w_width-90, 0, -10, 60, 5);
  Sound dataCable4 = new Sound("Data Cable4", color(255,0,0), w_width-120, 0, -10, 60, 5);
  Sound dataCable5 = new Sound("Data Cable5", color(255,0,0), w_width-150, 0, -10, 60, 5);
...
void setup() {
  ...
  midiBus = new MidiBus(this, -1, "Bus 1"); // Create a new MidiBus with no input device and the default Java Sound Synthesizer as the output device.
...
// ----------------------------------------------------------------
  ...
  */
  ...
  // Drone Highway
  if (playerPosition.z >= 5 && playerPosition.z <= 11) {
    ...
    if (timeElapsed == timeForDroneEncounter && droneEncounter == false){
      ...
      if (index == prevDroneIndex) drone = drones[int(random(drones.length))];
  ...
  // Data Cable
  /*
  dataCable1.initSound(20, height);
  dataCable2.initSound(20, height);
  dataCable3.initSound(20, height);
  dataCable4.initSound(20, height);
  dataCable5.initSound(20, height);
  */
...
void keyPressed() {
  ...
  if (key == 'a'){    //Emil's shortcut til action
  playerPosition.z = -10;
  }
